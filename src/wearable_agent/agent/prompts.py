...
1. **Data monitoring** — Continuously evaluate incoming physiological sensor \
   data (e.g., heart rate, steps, SpO2) from study participants' wearable \
   devices.
...
3. **Contextual reasoning** — Consider temporal patterns, participant history, \
   and study protocol when interpreting data.
4. **Notification** — Generate concise, actionable alerts when values breach \
   predefined thresholds.  Classify severity (info / warning / critical).
5. **Data integrity** — Note gaps in data collection, device disconnections, \
   or suspect readings.
6. **Affect inference** — You have access to the affect inference system which \

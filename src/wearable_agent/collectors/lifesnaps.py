    ...
    async def fetch(
        ...
        *,
        ...
        for _, row in daily_subset.iterrows():
            ...
            if MetricType.VO2_MAX in metrics and 'filteredDemographicVO2Max' in row and pd.notna(row['filteredDemographicVO2Max']):
                readings.append(SensorReading(
                    participant_id=participant_id,
                    device_type=DeviceType.FITBIT,
                    metric_type=MetricType.VO2_MAX,
                    value=float(row['filteredDemographicVO2Max']),
                    timestamp=ts
                ))

            if MetricType.SKIN_TEMPERATURE in metrics and 'nightly_temperature' in row and pd.notna(row['nightly_temperature']):
                readings.append(SensorReading(
                    participant_id=participant_id,
                    device_type=DeviceType.FITBIT,
                    metric_type=MetricType.SKIN_TEMPERATURE,
                    value=float(row['nightly_temperature']),
                    unit="C",
                    timestamp=ts
                ))

            if MetricType.SKIN_TEMPERATURE_VARIATION in metrics and 'daily_temperature_variation' in row and pd.notna(row['daily_temperature_variation']):
                readings.append(SensorReading(
                    participant_id=participant_id,
                    device_type=DeviceType.FITBIT,
                    metric_type=MetricType.SKIN_TEMPERATURE_VARIATION,
                    value=float(row['daily_temperature_variation']),
                    unit="C",
                    timestamp=ts
                ))

            if MetricType.RESTING_HEART_RATE in metrics and 'resting_hr' in row and pd.notna(row['resting_hr']):
                readings.append(SensorReading(
                    participant_id=participant_id,
                    device_type=DeviceType.FITBIT,
            ...
            if MetricType.SLEEP_EFFICIENCY in metrics and 'sleep_efficiency' in row and pd.notna(row['sleep_efficiency']):
                readings.append(SensorReading(
                    participant_id=participant_id,
                    device_type=DeviceType.FITBIT,
                    metric_type=MetricType.SLEEP_EFFICIENCY,
                    value=float(row['sleep_efficiency']),
                    unit="%",
                    timestamp=ts
                ))

            # Activity Minutes
            for m_type, col_name in [
                (MetricType.SEDENTARY_MINUTES, 'sedentary_minutes'),
                (MetricType.LIGHTLY_ACTIVE_MINUTES, 'lightly_active_minutes'),
                (MetricType.MODERATELY_ACTIVE_MINUTES, 'moderately_active_minutes'),
                (MetricType.VERY_ACTIVE_MINUTES, 'very_active_minutes')
            ]:
                if m_type in metrics and col_name in row and pd.notna(row[col_name]):
                    readings.append(SensorReading(
                        participant_id=participant_id,
                        device_type=DeviceType.FITBIT,
                        metric_type=m_type,
            ...
            # Affect Tags (EMA labels)
            # These are 1/0 flags. We verify their existence and if they are true (1).
            affect_cols = [
            ...
            if MetricType.AFFECT_TAG in metrics:
                    ...
                    if col in row and pd.notna(row[col]):
                        try:
                            ...
                            if val > 0:
                                readings.append(SensorReading(
                                    participant_id=participant_id,
                                    device_type=DeviceType.LIFESNAPS,
                                    metric_type=MetricType.AFFECT_TAG,
                                    value=val,
                                    unit="bool",
                                    timestamp=ts,
                                    metadata={"tag": col}
